...
export class NetworkStructureComponent implements OnInit, OnChanges{
  ...
  ngOnChanges(changes: SimpleChanges) {
  	if(this.isPlacingRouter) {
	    this.routerList.push({ id: this.routerIdCounter, x: this.deviceLocation.x, y: this.deviceLocation.y});
	    this.routerIdCounter++;
	    this.finishAdding.emit();
  	} else if(this.isPlacingHost) {
	    this.hostList.push({ id: this.hostIdCounter, x: this.deviceLocation.x, y: this.deviceLocation.y});
	    this.hostIdCounter++;
	    this.finishAdding.emit();
  	}
  	...
  	if(this.selectedRouter != router){
	    ...
	    switch(this.addLinkStatus) {
    		case AddLinkStatus.Ready:
        		this.linkTemp = {id: this.linkIdCounter, firstNodePos: {x:router.x+18, y:router.y+18}, secondNodePos: {x:router.x+18, y:router.y+18}};
        		this.addLinkStatus = AddLinkStatus.FisrtNodeSelected;
        		break;
    		case AddLinkStatus.FisrtNodeSelected:
    		    this.linkTemp.secondNodePos = {x:router.x+18, y:router.y+18};
    	...
    	this.selectedRouter = {id:-1, x:0, y:0};
    }
  }
